...
- [x] 4.3 (design D9) Expor `torchSupported`, `torchOn` e `toggleTorch()` em `useBarcodeScanner.ts`, checando `track.getCapabilities().torch` na track de vídeo ativa e usando `track.applyConstraints({ advanced: [{ torch }] })` para ligar/desligar; desligar automaticamente em `stop()`; adicionar o botão em `ScannerSpike.tsx`, visível só quando `torchSupported` for `true`; verificar que compila sem `any` e que o build passa
- [ ] 4.4 (Usuário) Testar o botão de lanterna em Android real: confirmar que aparece, liga/desliga o LED, e que ajuda a ler produtos com pouca luz que antes falhavam; confirmar no iPhone que o botão não aparece (Safari não suporta)
